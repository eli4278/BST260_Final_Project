...
```{r}
# fit model based on death counts from before 2017, use population as offset

training <- weekly_counts |> filter(year(date) < 2017)

fit <- glm(data = training, 
           outcome ~ week + as.factor(agegroup) + as.factor(sex) + log(population),
...
sum(pearson_residuals^2) / df.residual(fit)
```
...
pred_2017 <- predict.glm(fit, data_2017, se.fit = T)
```
